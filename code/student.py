...
def get_tiny_images(image_paths):
    ...
    for i in image_paths:
        ...
        calc_mean = image - np.mean(image)
        image = (calc_mean)/np.std(image)
        image = image.flatten()
...
def build_vocabulary(image_paths, vocab_size):
    ...
    for i in image_paths:
        read_image = imread(i)
        long_bois = hog(read_image, cells_per_block=(4, 4), pixels_per_cell= (8, 8), feature_vector= True)
        long_bois = np.reshape(long_bois,(-1, some_vocab))
        arr.append(long_bois)

    arr = np.vstack(arr)
    k_means = KMeans(n_clusters=vocab_size, max_iter=100).fit(arr)    
    load = np.vstack(k_means.cluster_centers_)
...
def get_bags_of_words(image_paths):
    ...
    for i in image_paths:
        ...
        long_bois = hog(read_image, cells_per_block=(4, 4), pixels_per_cell= (8, 8), feature_vector= True)
        long_bois = np.reshape(long_bois,(-1, some_vocab))
        distance = cdist(vocab, long_bois, metric = 'euclidean')
        mini = np.argmin(distance, axis = 0)
        histogram,_ = np.histogram(mini, bins = len(vocab))
...
def svm_classify(train_image_feats, train_labels, test_image_feats):
    ...
    SVC = linear_svc.fit(train_image_feats, train_labels)

    return np.array(SVC.predict(test_image_feats))
...
def nearest_neighbor_classify(train_image_feats, train_labels, test_image_feats):
    ...
    for i in np.array(distances):
        dist = np.argsort(i)
        index = train_labels[dist]
        index = index[0 : k]
        label = mode(index, keepdims= True)
        arr.append(label.mode)
